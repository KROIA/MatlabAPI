...
namespace MatlabAPI
{
	...
	StateSpaceModel TransferFunction::toStateSpaceModel() const
	{
		...
		MatlabEngine::eval(("sysd = c2d(sys, " + std::to_string(dt) + ", 'zoh'); [Ad,Bd,Cd,Dd] = ssdata(ss(sysd)); [A,B,C,D] = ssdata(ss(sys));").c_str());
